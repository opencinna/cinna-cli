...
| **Agent addons** | `cinna skills` (list, publish) — installed plugins + the agent's own `skills/<name>/` folders, publishing one to the skills catalog, and the versions the catalog derives and writes back into `SKILL.md` | [business](features/agent_addons/agent_addons.md) · [tech](features/agent_addons/agent_addons_tech.md) · [acceptance](features/agent_addons/agent_addons_acceptance.md) |
